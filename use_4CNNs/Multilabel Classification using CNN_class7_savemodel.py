...
image_paths = glob.glob("Posters/*.jpg")
...
classes = list_genres(7)
classes = set(classes)
print('classes:',classes)
print(len(classes))
y = list_movies(classes,image_ids)
...
def model(trainx, trainy, testx, testy, learning_rate = 0.009, epochs = 30, isPlot=False):
    ...
    with tf.Session() as sess:
        ...
        for i in range(epochs):
            ...
            for minibatch in minibatches:
                ...
                cnt += 1
                if cnt %10 == 0:
                    print('minibatch_cost:',temp)
            lcost = lcost/nums
            if i%10 == 0:
                print("当前是第 " + str(i) + " 代，成本值为：" + str(lcost))
                costs.append(lcost)
        
        #保存学习后的参数
        parameters = sess.run(params)
        print("参数已经保存到session。")
        saver.save(sess,"model/save_net.ckpt",global_step=i)
        ...
        print("训练集准确度：" + str(train_acc))
        print("测试集准确度：" + str(test_acc))
        if isPlot:
...
_, _, params = model(trainx, trainy, testx, testy, True)
